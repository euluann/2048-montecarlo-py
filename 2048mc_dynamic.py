...
# moves_limit - Limite de movimentos do clone
moves_limit = 10

# clone_per_score - Quantidade de clones por score
clone_per_score = 0.002

# clones_min - Quantidade minima de clones
clones_min = 10

# clones_max - Quantidade minima de clones
clones_max = 60
...
class System(App):
	def build(self):
		...
		# Imprime o titulo "Monte Carlo" na tela com fonte especifica e com tamanho de fonte relativa a altura da tela
		#self.mc_title_label = Label(text=f"Monte Carlo", pos=(0,-size/2-height*0.03), font_size=height*0.04, font_name="data/font/font.ttf", color=(*colors_2048["black_text"], 1))
#		layout.add_widget(self.mc_title_label)
		
		self.clones_title_label = Label(text=f"Clones", pos=(0,-size/2-height*0.04), font_size=height*0.04, font_name="data/font/font.ttf", color=(*colors_2048["black_text"], 1))
		layout.add_widget(self.clones_title_label)
		self.clones_label = Label(text=f"{clones_min}", pos=(0,-size/2-height*0.09), font_size=height*0.04, font_name="data/font/font.ttf", color=(*colors_2048["black_text"], 1))
		layout.add_widget(self.clones_label)
	...
	def update_moves(self, dt):
		# Verifica se nao ha movimento acontecendo
		if not self.board_while_animation:
			
			# Calcula a quantidade de clones para movimento atual
			clones = min(int(clone_per_score*self.score)+clones_min, clones_max)
			
			# Atualiza o texto com a quantidade atual de clones
			self.clones_label.text = f"{clones}"
			self.clones_label.texture_update()
			
			# Implementa o algoritmo de monte carlo retornando o melhor movimento baseado nos seus testes
			move = back2048.monte_carlo_2048(self.board, moves_limit, clones)
			
			# Aplica o respectivo movimento
			if move == "up":
